...
namespace CRH.Framework.Disk
{
    public sealed class DiskReader : Disk
    {
        ...
        private bool m_cueDataTrackSet;
    ...
    // Constructors
            ...
            : base(fileUrl, system)
        {
            try
            {
                ...
                FileStream cueFileStream = new FileStream(cueFile.FullName, FileMode.Open, FileAccess.Read, FileShare.Read);
                StreamReader cueStream   = new StreamReader(cueFileStream);
                m_cueDataTrackSet = false;
                ...
                {
                    keyWord = m_regCueKeyWord.Match(line).Groups[1].Value;
                    ...
                    switch (keyWord)
                    {
                        ...
                        case "TRACK":
                            ...
                            if((!mode.StartsWith("MODE") && !m_cueDataTrackSet))
                            ...
                            else if((mode.StartsWith("MODE") && m_cueDataTrackSet))
                        ...
                        case "INDEX":
                            ...
                            if(indexNumber == 0)
                                track.IsPregapStored = true;
                            else if(indexNumber == 2)
                                track.IsPostgapStored = true;
                ...
                {
                    track = m_tracks[i];
                    if (track.IsPregapStored)
                    {
                        track.PregapOffset = indexOffsets[u];
                        track.PregapSize = (uint)((indexOffsets[u + 1] - indexOffsets[u]) / track.SectorSize);
                        u++;
                    }
                    ...
                    if (track.IsPostgapStored)
                    {
                        track.PostgapOffset = indexOffsets[u];
                        track.PostgapSize = (uint)((indexOffsets[u + 1] - indexOffsets[u]) / track.SectorSize);
                        u++;
                    }
                }
    ...
    // Methods
        ...
        /// <param name="fileUrl">Path to the CUE file to read</param>
        /// <param name="system">File system used for data track</param>
        public static DiskReader InitMultiTracks(string fileUrl, DiskFileSystem system)
        {
        ...
        /// <param name="readDescriptors">Read descriptors immediately</param>
        /// <param name="buildIndex">Build the index cache immediately</param>
        public static DiskReader InitSingleTrack(string fileUrl, DiskFileSystem system, DataTrackMode mode, bool readDescriptors = true, bool buildIndex = true)
        {
